...
def group_vq_features(
    ...
    # 将特征按间隔划分为R、G、B通道
    features_r = features[:, ::3]  # R通道 (0, 3, 6, ...)
    features_g = features[:, 1::3]  # G通道 (1, 4, 7, ...)
    features_b = features[:, 2::3]  # B通道 (2, 5, 8, ...)

    # 归一化importance
    importance_n = importance / importance.max()

    # 为每个RGB通道创建一个单独的VQ模型
    vq_model_r = VectorQuantize(
        channels=features_r.shape[1],
        codebook_size=codebook_size,
        decay=decay,
    ).to(device=features.device)

    vq_model_g = VectorQuantize(
        channels=features_g.shape[1],
        codebook_size=codebook_size,
        decay=decay,
    ).to(device=features.device)

    vq_model_b = VectorQuantize(
        channels=features_b.shape[1],
        codebook_size=codebook_size,
        decay=decay,
    ).to(device=features.device)

    # 初始化码本
    vq_model_r.uniform_init(features_r)
    vq_model_g.uniform_init(features_g)
    vq_model_b.uniform_init(features_b)

    # 训练每个码本
    for i in trange(steps, desc="RGB Group VQ training"):
        batch = torch.randint(low=0, high=features.shape[0], size=[vq_chunk])

        error_r = vq_model_r.update(features_r[batch], importance=importance_n[batch])
        error_g = vq_model_g.update(features_g[batch], importance=importance_n[batch])
        error_b = vq_model_b.update(features_b[batch], importance=importance_n[batch])

    # 获取VQ索引
    _, indices_r = vq_model_r(features_r)
    _, indices_g = vq_model_g(features_g)
    _, indices_b = vq_model_b(features_b)

    return (
        vq_model_r.codebook.data.detach(),
        vq_model_g.codebook.data.detach(),
        vq_model_b.codebook.data.detach(),
        torch.stack([indices_r, indices_g, indices_b], dim=-1).detach(),
    )
...
def join_features_group_vq(
    ...
    """
    # 将原始特征按间隔拆分为 R、G、B 通道
    features_r = all_features[:, ::3]
    features_g = all_features[:, 1::3]
    features_b = all_features[:, 2::3]

    # 获取保留的特征
    keep_features_r = features_r[keep_mask]
    keep_features_g = features_g[keep_mask]
    keep_features_b = features_b[keep_mask]

    # 码本拆分
    codebook_r, codebook_g, codebook_b = codebooks
    M = codebook_r.shape[0]  # 假设每个码本的大小相同
    ...
    for i in range(0, M, batch_size):
        ...
        for j in range(0, M, batch_size):
            ...
            for k in range(0, M, batch_size):
                ...
                # 提取当前批次的 R、G、B 码本
                r_batch = codebook_r[i:end_i]  # 形状 (batch_size, 16)
                g_batch = codebook_g[j:end_j]  # 形状 (batch_size, 16)
                b_batch = codebook_b[k:end_k]  # 形状 (batch_size, 16)

                # 广播并组合 R、G、B 码本，生成形状为 (batch_size_r * batch_size_g * batch_size_b, 48)
                r_batch = r_batch.unsqueeze(1).unsqueeze(2)  # (batch_size_r, 1, 1, 16)
                g_batch = g_batch.unsqueeze(0).unsqueeze(2)  # (1, batch_size_g, 1, 16)
                b_batch = b_batch.unsqueeze(0).unsqueeze(1)  # (1, 1, batch_size_b, 16)

                # 创建组合特征
                batch_combined = torch.empty(
                    (r_batch.size(0), g_batch.size(1), b_batch.size(2), 48), device=r_batch.device
                )
                batch_combined[..., ::3] = r_batch.expand(-1, g_batch.size(1), b_batch.size(2), -1)
                batch_combined[..., 1::3] = g_batch.expand(r_batch.size(0), -1, b_batch.size(2), -1)
                batch_combined[..., 2::3] = b_batch.expand(r_batch.size(0), g_batch.size(1), -1, -1)

                # 展平组合后的结果，形状为 (batch_size_r * batch_size_g * batch_size_b, 48)
                batch_combined = batch_combined.view(-1, 48)
    ...
    # 将保留的特征合并到大码本中
    keep_features = torch.cat([keep_features_r, keep_features_g, keep_features_b], dim=-1)
