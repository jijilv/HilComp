...
def compress_color(
    ...
    codebook_features, indices, codebook_prob = join_features(
        all_features, keep_mask, color_codebook, color_vq_indices
    )

    # gaussians.set_color_indexed(codebook_features.reshape(-1, n_sh_coefs, 3), indices)

    return codebook_features, indices, codebook_prob
...
def compress_covariance(
    ...
    codebook_cov, cov_indices, codebook_prob = join_features(
        covariance,
        keep_mask_g,
        cov_codebook,
        cov_vq_indices,
    ...
    # rot_vq, scale_vq = extract_rot_scale(to_full_cov(codebook_cov))

    # gaussians.set_gaussian_indexed(
    #     rot_vq.to(codebook_cov.device),
    #     scale_vq.to(codebook_cov.device),
    #     cov_indices,
    # )

    return codebook_cov, cov_indices, codebook_prob
...
def compress_gaussians(
    ...
    with torch.no_grad():
        ...
        color_codebook, color_indices, color_prob = None, None, None
        cov_codebook, cov_indices, cov_prob = None, None, None
        
        # 压缩颜色特征
        if color_comp is not None:
            color_codebook, color_indices, color_prob = compress_color(
                gaussians,
                color_importance,
                color_comp,
        ...
        if gaussian_comp is not None:
            cov_codebook, cov_indices, cov_prob = compress_covariance(
                gaussians,
                gaussian_importance,
                gaussian_comp,
            )
    ...
    # 返回需要的值
    return color_codebook, color_indices, color_prob, cov_codebook, cov_indices, cov_prob
