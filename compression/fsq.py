...
class FSQ(Module):
    ...
    #     # 使用正态分布初始化权重，指定均值和标准差
    #     nn.init.normal_(self.linear1.weight, mean=mean, std=std)
    #     nn.init.normal_(self.linear2.weight, mean=mean, std=std)

    #     # 你也可以初始化偏差
    #     if self.linear1.bias is not None:
    #         nn.init.normal_(self.linear1.bias, mean=mean, std=std)
    #     if self.linear2.bias is not None:
    #         nn.init.normal_(self.linear2.bias, mean=mean, std=std)
