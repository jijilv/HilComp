...
from compression.lvq import CompressionSettings, compress_gaussians
from gaussian_renderer2 import GaussianModel, render
...
def run_vq(
    ...
    with torch.no_grad():
        ...
        color_codebook, color_indices, color_prob, cov_codebook, cov_indices, cov_prob = compress_gaussians(
            gaussians,
            color_importance_n,
        ...
        gaussians.set_color(color_codebook.reshape(-1, 16, 3), color_indices)

        rot_vq, scale_vq = extract_rot_scale(to_full_cov(cov_codebook))
        gaussians.set_cov(
            rot_vq.to(cov_codebook.device),
            scale_vq.to(cov_codebook.device),
            cov_indices,
        )

        gaussians.set_color_codebook(color_codebook, color_indices, color_prob)

        gaussians.set_cov_codebook(cov_codebook,cov_indices,cov_prob)
    ...
    with open(f"{comp_params.output_vq}/times.json","w") as f:
        json.dump(timings,f)
    file_size = get_folder_size(out_file) / (1024 ** 2)
